...
\begin{table}[t]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Keyword / regex baseline      & $\approx 0.115$ & $\approx 0.115$ & $\approx 0.56$ \\
    Action-perplexity baseline    & $\approx 0.08$  & ---             & ---           \\
  ...
  are run through the identical harness. The action-perplexity baseline's
  PR@FSR=5\% and AUROC were not logged for the v0.19 run (shown as ---).
  }
  \label{tab:main}
\end{table}
...
\subsection{Latency}
\label{sec:experiments:latency}

The monitor adds a single forward pass per proposed action. On the training GPU
(an RTX 6000 Pro, Blackwell), a \texttt{check()} on a 20-step trace costs
$\approx 227$\,ms at p50 ($\approx 228$\,ms p90) via the Unsloth fast-inference
path; on the common \textsc{OK} path no text is decoded, so this is essentially
one prefill. Since an agent's own per-step cost (the policy-LLM call plus tool
I/O) is typically $100$\,ms--seconds, the guard fits comfortably inside that
budget. We additionally verified a KV-cached incremental session that returns
\emph{numerically identical} $P(\textsc{STOP})$ to the uncached path (maximum gap
$0.000$ over a session); it did not yield a wall-clock speedup in our
environment, so we report the uncached figure. Quantitative cross-backend
transfer and the training ablations are not yet measured
