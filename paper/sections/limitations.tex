...
\begin{itemize}[leftmargin=1.4em,itemsep=0.2em]
  ...
  \item \textbf{Per-family imbalance.} Detection is near-perfect on shaped
        ...
        families.
  \item \textbf{Latency measured on one GPU only.} A \texttt{check()} runs at
        p50 $\approx 227$\,ms on a 20-step trace on an RTX 6000 Pro
        (Section~\ref{sec:experiments:latency}). We have not characterized CPU /
        edge latency, smaller or quantized GPUs, or the KV-cache speedup (which
        was numerically correct but did not accelerate in our environment).
  ...
  \item \textbf{Evasion.} As a learned distributional monitor, IAM is in
        ...
        not yet run an adaptive-attacker evaluation.
