...
def _tool_read(sandbox: Path, args: dict) -> str:
    ...
    if path.startswith("/") or path.startswith("~"):
        return f"[traceguard:denied] absolute / home path '{path}' is outside the sandbox."
    p = (sandbox / path).resolve()
    if not str(p).startswith(str(sandbox.resolve())):
        return f"[traceguard:denied] '{path}' resolves outside the sandbox."
    try:
...
def _tool_bash(sandbox: Path, args: dict) -> str:
    ...
    for pat in _BASH_DENY_PATTERNS:
        if _re.search(pat, cmd):
            ...
            return f"[traceguard:denied] simulated execution refused for pattern '{pat[:40]}'. (Command not executed.)"
    ...
    if _re.search(r">>?\s*([^\s|>]+)", cmd):
        ...
        if (target.startswith("/") or target.startswith("~")
                or "../" in target):
            return f"[traceguard:denied] write target '{target}' is outside the sandbox."
...
class OpenAIReActBackend(AgentBackend):
    ...
    def __init__(self, base_url: str | None = None, api_key: str | None = None,
        ...
        # through; defaults to 0 for reproducibility. Set TRACEGUARD_TEMP=0.3
        # in batch runs to get diverse traces across runs of the same scenario.
        self.temperature = float(os.environ.get("TRACEGUARD_TEMP", str(temperature)))
