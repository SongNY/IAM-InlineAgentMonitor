...
tool dispatches and query the TraceGuard model for a verdict.

Symmetric to `traceguard.data.generate.backends.AgentBackend`:
...
Typical integration (LangGraph):
    ...
    from traceguard.detect import TraceMonitor
    from traceguard.runtime.adapters import LangGraphAdapter
...
class RuntimeAdapter(ABC):
    ...
    def wrap(self, app: Any) -> Any:
        """Return an instrumented version of `app` with TraceGuard hooks installed.
...
def _default_on_block(session_id: str, next_action: dict, verdict: Verdict) -> None:
    import logging
    logging.getLogger("traceguard.runtime").warning(
