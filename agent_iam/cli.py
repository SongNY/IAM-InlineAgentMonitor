"""Minimal CLI: `iam scan <jsonl>` and `iam serve`."""
...
@click.group()
def main():
    """IAM — trace anomaly detection for LLM agents."""


@main.command()
@click.argument("path", type=click.Path(exists=True, dir_okay=False))
@click.option("--model", default="Sunnyu/IAM-Qwen3.5-2B")
@click.option("--threshold", default=8.0, type=float)
def scan(path: str, model: str, threshold: float):
    """Run IAM over each trajectory in a JSONL file."""
...
def serve(host: str, port: int, model: str):
    """Run IAM as a sidecar HTTP service."""
    import uvicorn
