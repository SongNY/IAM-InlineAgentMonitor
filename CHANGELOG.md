...
- **Canonical `Trajectory` schema** (`agent_iam.schema`) with an ATBench-compatible label
...
- **`TraceMonitor`** (`agent_iam.detect.online`) — online mid-trace inference:
  `from_pretrained`, `check` (verdict-head `P(STOP)` in the dense per-step layout),
  `verdict_at`, and a legacy action-span `ppl_score`. Lazy-loaded so `import agent_iam`
  stays light (no torch required for the schema/tokenizer).
- **`StreamingMonitor`** (`agent_iam.runtime.stream`) — agent-loop API
...
- **Eval harness** (`agent_iam.eval`) — deployment-framed metrics (Prevention Rate, False
...
- **Data-generation pipeline** (`scripts/` + `agent_iam.data`) — scenario synthesis,
...
- **Runtime adapters** (`agent_iam.runtime.adapters`) for Claude Code and LangGraph, plus
  the `agent_iam` CLI (`scan`, `serve`).
